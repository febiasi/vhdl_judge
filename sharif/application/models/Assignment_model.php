		...
		for ($i=1; $i<=$this->input->post('number_of_problems'); $i++)
		{
			...
			foreach ($items as $item){
				...
				elseif ($item2 === 'vhdl')
					$item = 'VHDL';
				$item2 = strtolower($item);
				if ( ! in_array($item2, array('c','c++','python 2','python 3','java','vhdl','zip','pdf')))
					continue;
				// If the problem is not Upload-Only, its language should be one of {C,C++,Python 2, Python 3,Java,VHDL}
				if ( ! in_array($i, $uo) && ! in_array($item2, array('c','c++','python 2','python 3','java','vhdl')) )
					continue;
				$ft[$i-1] .= $item.",";
			}
